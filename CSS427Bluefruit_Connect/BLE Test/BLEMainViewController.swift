...
class BLEMainViewController : UIViewController, UINavigationControllerDelegate, HelpViewControllerDelegate, CBCentralManagerDelegate,
                              BLEPeripheralDelegate, UARTViewControllerDelegate, CSS427BackupSensorDelegate, PinIOViewControllerDelegate, DeviceListViewControllerDelegate, FirmwareUpdaterDelegate {
    ...
    var backupSensorViewController:CSS427BackupSensorViewController!
    ...
    var currentConnectingPeripheral: BLEPeripheral?
    ...
    private var sensorPeripheral:BLEPeripheral?
    private var receiverPeripheral:BLEPeripheral?
...
//            var index = -1
    ...
    func currentHelpViewController()->HelpViewController {
        ...
        else if navController.topViewController!.isKindOfClass(CSS427BackupSensorViewController) {
            hvc = backupSensorViewController.helpViewController
        }
    ...
    func connectPeripheral(peripheral:CBPeripheral, mode:ConnectionMode) {
        ...
        let devices = defaults.valueForKey("devices") as! [String:String]
        
        if devices[peripheral.identifier.UUIDString] == nil {
            return
        }
        if (devices[peripheral.identifier.UUIDString] == "Sensor") {
            sensorPeripheral = BLEPeripheral(peripheral: peripheral, delegate: self)
        }
        else if (devices[peripheral.identifier.UUIDString] == "Receiver") {
            receiverPeripheral = BLEPeripheral(peripheral: peripheral, delegate: self)
        }
        else { return; }
    ...
    func abortConnection() {
        ...
        connectionTimer?.invalidate()
        if (deviceListViewController.currentConnectingDeviceType == nil) { return; }
        
        if deviceListViewController.currentConnectingDeviceType == .Sensor {
            if (cm != nil) && (sensorPeripheral != nil) {
                cm!.cancelPeripheralConnection(sensorPeripheral!.currentPeripheral)
            }
            sensorPeripheral = nil
        }
        else {
            if (cm != nil) && (receiverPeripheral != nil) {
                cm!.cancelPeripheralConnection(receiverPeripheral!.currentPeripheral)
            }
            receiverPeripheral = nil
        }
    ...
    func disconnectAll() {
        if (sensorPeripheral != nil) {
            disconnect(sensorPeripheral!.currentPeripheral.identifier.UUIDString)
        }
        if (receiverPeripheral != nil) {
            disconnect(receiverPeripheral!.currentPeripheral.identifier.UUIDString)
        }
    }
    ...
    func disconnect(uuid: String) {
        ...
        var thePeripheral: BLEPeripheral!
        
        if sensorPeripheral != nil && receiverPeripheral != nil {
            if (sensorPeripheral!.currentPeripheral.identifier.UUIDString == uuid) {
                thePeripheral = sensorPeripheral
            }
            else if (receiverPeripheral!.currentPeripheral.identifier.UUIDString == uuid) {
                thePeripheral = receiverPeripheral
            }
        }
        else if (sensorPeripheral != nil) {
            thePeripheral = sensorPeripheral
        }
        else if (receiverPeripheral != nil) {
            thePeripheral = receiverPeripheral
        }
    ...
    func alertDismissedOnError() {
        ...
        if (connectionStatus == ConnectionStatus.Connected) {
            disconnect(self.currentConnectingPeripheral!.currentPeripheral.identifier.UUIDString)
        }
    ...
    func centralManager(central: CBCentralManager, didConnectPeripheral peripheral: CBPeripheral) {
        let peripheralType = deviceListViewController.currentConnectingDeviceType
        
        if (peripheralType == nil) { return; }
        var thePeripheral: BLEPeripheral!
        ...
        if thePeripheral!.currentPeripheral == peripheral {
            ...
            if((peripheral.services) != nil){
                ...
                thePeripheral!.peripheral(peripheral, didDiscoverServices: nil)  //already discovered services, DO NOT re-discover. Just pass along the peripheral.
            }
            else {
                thePeripheral!.didConnect(connectionMode)
            }
    ...
    func dereferenceModeController() {
        ...
        backupSensorViewController = nil
    ...
    func isModuleController(anObject:AnyObject)->Bool{
        ...
        if     anObject.isMemberOfClass(PinIOViewController)
            ...
            || anObject.isMemberOfClass(CSS427BackupSensorViewController)
    ...
    func connectionFinalized() {
        ...
        if (firmwareUpdater != nil && connectionMode != .DFU) {
            ...
            firmwareUpdater!.checkUpdatesForPeripheral(thePeripheral!.currentPeripheral, delegate: self)
        }
        else if (sensorPeripheral != nil && receiverPeripheral != nil) {
            // Automatic updates not enabled. Just go to the mode selected by the user
            launchViewControllerForSelectedMode()
            deviceListViewController.didConnectPeripheral(thePeripheral.currentPeripheral)
        }
        else if (sensorPeripheral != nil || receiverPeripheral != nil) {
    ...
    func launchViewControllerForSelectedMode() {
        ...
        case ConnectionMode.PinIO:
            ...
            break
        case ConnectionMode.BackupSensor:
            let aStoryboard = UIStoryboard(name: "CSS427Storyboard", bundle: NSBundle.mainBundle())
            
            backupSensorViewController = aStoryboard.instantiateViewControllerWithIdentifier("backupSensorController") as! CSS427BackupSensorViewController
            backupSensorViewController.setupBluetoothDevices(sensorPeripheral?.currentPeripheral.identifier.UUIDString, receiver: receiverPeripheral?.currentPeripheral.identifier.UUIDString)
            backupSensorViewController.delegate = self
            backupSensorViewController.didConnect()
            vc = backupSensorViewController
            break
        ...
        case ConnectionMode.Info:
            ...
            deviceInfoViewController = DeviceInfoViewController(cbPeripheral: thePeripheral!.currentPeripheral, delegate: self)
...
//        let peripheralType = deviceListViewController.currentConnectingDeviceType
        ...
        thePeripheral!.writeRawData(newData)
        
    }
    
    //For only connecting to one device
...
//        navController.popToRootViewControllerAnimated(true)
    ...
    func showUpdateAvailableForRelease(latestRelease: FirmwareInfo!) {
        ...
        alert.addAction(UIAlertAction(title: "Go to updates", style: UIAlertActionStyle.Default, handler: { _ in
            ...
            let peripheralType = self.deviceListViewController.currentConnectingDeviceType
            
            if (peripheralType == nil) { return; }
            var thePeripheral: BLEPeripheral!
            
            if (peripheralType == .Sensor) {
                thePeripheral = self.sensorPeripheral
            }
            else {
                thePeripheral = self.receiverPeripheral
            }
            
            self.launchDFU(thePeripheral.currentPeripheral)
        }))
