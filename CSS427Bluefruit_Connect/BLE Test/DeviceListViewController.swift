...
class DeviceListViewController : UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    internal var currentConnectingDeviceType: DeviceType?
...
//        alertController.addAction(aaInfo)
        ...
        if (device.isUART) {
            let sensorController = UIAlertAction(title: "Sensor", style: .Default, handler: { (action) in
                self.currentConnectingDeviceType = .Sensor
                self.nameDeviceBeforeConnecting(device)
                self.connectInMode(.BackupSensor, peripheral: device.peripheral)
                
            })
           
            
            let receiverController = UIAlertAction(title: "Receiver", style: .Default, handler: { (action) in
                self.currentConnectingDeviceType = .Receiver
                self.nameDeviceBeforeConnecting(device)
                self.connectInMode(.BackupSensor, peripheral: device.peripheral)
                
            })
            if (device.name == "Sensor" || device.name == "Receiver") {
                let connectAction = UIAlertAction(title: "Reconnect", style: .Default, handler: { (action) in
                    if (device.name == "Sensor") { self.currentConnectingDeviceType = .Sensor }
                    else if (device.name == "Receiver") { self.currentConnectingDeviceType = .Receiver }
                    self.connectInMode(.BackupSensor, peripheral: device.peripheral)
                })
                alertController.addAction(connectAction)
            }
            alertController.addAction(sensorController)
            alertController.addAction(receiverController)
        }
    ...
    func nameDeviceBeforeConnecting(device: BLEDevice) {
        if (currentConnectingDeviceType != nil) {
            let defaults = NSUserDefaults.standardUserDefaults()
            var allDevices = defaults.valueForKey("devices") as! [String:String]
            
            if currentConnectingDeviceType == .Sensor {
                allDevices[device.peripheral.identifier.UUIDString] = "Sensor"
            }
            else if currentConnectingDeviceType == .Receiver {
                allDevices[device.peripheral.identifier.UUIDString] = "Receiver"
            }
            
            defaults.setValue(allDevices, forKey: "devices")
        }
    }
...
//                println("   \(self.classForCoder.description()) updating device RSSI")
        ...
        let defaults = NSUserDefaults.standardUserDefaults()
        if (defaults.valueForKey("devices") == nil) {
            let devices = [String:String]()
            defaults.setValue(devices, forKey: "devices")
        }
        var devicesIdsAndNames = defaults.valueForKey("devices") as! [String:String]
        if (devicesIdsAndNames[newID.UUIDString] == nil && peripheral.name != nil) {
            devicesIdsAndNames[newID.UUIDString] = peripheral.name!
        }
        else if (devicesIdsAndNames[newID.UUIDString] == nil) {
            devicesIdsAndNames[newID.UUIDString] = "Unknown Device"
        }
        defaults.setValue(devicesIdsAndNames, forKey: "devices")
        
        
        
        //Add reference to new device
        let newDevice = BLEDevice(peripheral: peripheral, advertisementData: advertisementData, RSSI: RSSI)
        if (devicesIdsAndNames[newID.UUIDString] != nil) {
            newDevice.name = devicesIdsAndNames[newID.UUIDString]!
        }
    ...
    func didConnectPeripheral(peripheral:CBPeripheral!) {
        ...
        currentConnectingDeviceType = nil
